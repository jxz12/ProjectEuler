...
class Problem50
{
    ...
    {
        List<long> primes = Erastosthenes(6000000); // kinda stupid magic number
        ...
        while (t-- > 0)
        {
            ...
            {
                if (primes[i] > 131) break; // really stupid magic number

                // for each starting prime, find biggest sum below n
                long sum = 0;
                int j = i;
                ...
                int steps = j-i;
                while (steps > bestSteps || (steps==bestSteps && sum<bestSum))
                {
